	...
	if (arg)
	{
	    ...
	    for_each_online_cpu(i) {
		    ...
		    */
            ...
            user = after[i].user;
            system = after[i].system;
            idle = after[i].idle;
            irq = (after[i].irq + after[i].softirq);      // separate irq and softirq?
            total = user + system + irq ; //total CPU usage
            dynamic = system + irq;       //dynamic CPU usage
        ...
        if(dynamic_turn == 0){               // turn for blk
            if(blk_queue_util > (T_HIGH*(BLK_END-blk_start+1))){
                if(blk_queue_util/(BLK_END-blk_start+1)>net_sys_util/(blk_start-NET_START-1)){
...
/*
            if(net_queue_util > (T_HIGH*(net_end-NET_START+1))){
                if(net_sys_util > (T_HIGH*(blk_start-net_end-1))){
                    if(blk_queue_util/(BLK_END-blk_start+1) <= (net_sys_util+net_queue_util)/(blk_start-NET_START)){
                        net_end = net_end + 1;
                        blk_start = blk_start+ 1;
                    }
                    else if(blk_queue_util/(BLK_END-blk_start+1) > (net_sys_util+net_queue_util)/(blk_start-NET_START)){
                        if(net_queue_util/(net_end-NET_START+1) <= net_sys_util/(blk_start-net_end-1)){
                            if(blk_start-net_end >= 3){
                                net_end = net_end + 1;
                            }
                            else if(blk_start-net_end < 3){
                                //pass
                            }
                        }
                        else if(net_queue_util/(net_end-NET_START+1) < net_sys_util/(blk_start-net_end-1)){
                            ...
                            else if(net_end > NET_START){
                                //pass
                            }
                        }
                    }
                }
                else if(net_sys_util < (T_LOW*(blk_start-net_end-1))){
                    if(blk_start-net_end >= 3){
                        net_end = net_end + 1;
                    }
                    else if(blk_start - net_end < 3){
...
*/
        ...
        if(T_HIGH < blk_queue_util/blk_core_counter && T_HIGH < net_queue_util/net_core_counter && T_HIGH < net_sys_util/netsys_core_counter){
        ...
        //if low,  sockets--
        else if(T_LOW > blk_queue_util/blk_core_counter && T_LOW > net_queue_util/net_core_counter && T_LOW > net_sys_util/netsys_core_counter){
            if(socket_number > 1){
                socket_number = socket_number - 1;
            }
        ...
        for(i = 1 ; i <= socket_number ; i++){
            if(dynamic_turn == 0){               // turn for blk
                if(blk_queue_util > (T_HIGH*(blk_end_per[NUMBER_OF_SOCKETS - i]-blk_start_per[NUMBER_OF_SOCKETS - i]+1))){
                    if(blk_queue_util/(blk_end_per[NUMBER_OF_SOCKETS - i]-blk_start_per[NUMBER_OF_SOCKETS - i]+1) > net_sys_util/(blk_start_per[NUMBER_OF_SOCKETS - i] - net_end_per[NUMBER_OF_SOCKETS - i] - 1)){
                    ...
                    else if(blk_queue_util/(blk_end_per[NUMBER_OF_SOCKETS - i]-blk_start_per[NUMBER_OF_SOCKETS - i]+1) < net_sys_util/(blk_start_per[NUMBER_OF_SOCKETS - i] - net_end_per[NUMBER_OF_SOCKETS - i] - 1)){
                        if(blk_start_per[NUMBER_OF_SOCKETS - i] != blk_end_per[NUMBER_OF_SOCKETS - i]){
                            blk_start_per[NUMBER_OF_SOCKETS - i] = blk_start_per[NUMBER_OF_SOCKETS - i] + 1;
                        }
                    }
                }
                else if((blk_queue_util < (T_LOW*(blk_end_per[NUMBER_OF_SOCKETS - i]-blk_start_per[NUMBER_OF_SOCKETS - i]+1)))){
                ...
                if(net_queue_util > (T_HIGH*(net_end_per[NUMBER_OF_SOCKETS - i]-net_start_per[NUMBER_OF_SOCKETS - i]+1))){
                    if(net_queue_util / (net_end_per[NUMBER_OF_SOCKETS - i]-net_start_per[NUMBER_OF_SOCKETS - i]+1) > net_sys_util / (blk_start_per[NUMBER_OF_SOCKETS - i] - net_end_per[NUMBER_OF_SOCKETS - i] - 1)){
                        if(blk_start_per[NUMBER_OF_SOCKETS - i] - net_end_per[NUMBER_OF_SOCKETS - i] > 3){
                            net_end_per[NUMBER_OF_SOCKETS - i] = net_end_per[NUMBER_OF_SOCKETS - i] + 1;
                        }
                    }
                    else if(net_queue_util / (net_end_per[NUMBER_OF_SOCKETS - i]-net_start_per[NUMBER_OF_SOCKETS - i]+1) < net_sys_util / (blk_start_per[NUMBER_OF_SOCKETS - i] - net_end_per[NUMBER_OF_SOCKETS - i] - 1)){
                        if(net_start_per[NUMBER_OF_SOCKETS - i] != net_end_per[NUMBER_OF_SOCKETS - i]){
                            net_end_per[NUMBER_OF_SOCKETS - i] = net_end_per[NUMBER_OF_SOCKETS - i] - 1;
                        }
                    }
                }
                else if(net_queue_util < (T_LOW*(blk_start_per[NUMBER_OF_SOCKETS - i]-net_end_per[NUMBER_OF_SOCKETS - i]-1))){
                    if(net_start_per[NUMBER_OF_SOCKETS - i] != net_end_per[NUMBER_OF_SOCKETS - i]){
                        net_end_per[NUMBER_OF_SOCKETS - i] = net_end_per[NUMBER_OF_SOCKETS - i] - 1;
                    }
                }
